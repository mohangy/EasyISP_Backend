...
nasRoutes.get('/', requirePermission('routers:view'), async (c) => {
    ...
    const [routers, total] = await Promise.all([
        prisma.nAS.findMany({
            where: where as Parameters<typeof prisma.nAS.findMany>[0]['where'],
            include: {
                _count: {
        ...
        prisma.nAS.count({ where: where as Parameters<typeof prisma.nAS.count>[0]['where'] }),
    ]);
