...
sessionRoutes.get('/stats', async (c) => {
    ...
    const [activeSessions, pppoeActive, hotspotActive, todaySessions, weekSessions, monthSessions, byNas,] = await Promise.all([
    ...
    const nasIds = byNas.map((n) => n.nasId).filter((id) => id !== null);
    const nasDevices = await prisma.nAS.findMany({
        where: { id: { in: nasIds }, tenantId },
        select: { id: true, name: true },
    });
    const nasMap = new Map(nasDevices.map((n) => [n.id, n.name]));
    return c.json({
