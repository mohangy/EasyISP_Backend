...
financeRoutes.post('/income', async (c) => {
    ...
    const payment = await prisma.payment.create({
        data: {
            amount: data.amount,
            method: (methodMap[data.method] ?? 'OTHER'),
            status: 'COMPLETED',
...
financeRoutes.get('/invoices', async (c) => {
    ...
    const total = await prisma.invoice.count({ where: where });
    const statsSummary = { total: 0, paid: 0, pending: 0, overdue: 0 };
    stats.forEach((s) => {
        const amount = s._sum.amount ?? 0;
        statsSummary.total += amount;
        if (s.status === 'paid')
            statsSummary.paid += amount;
        if (s.status === 'pending')
            statsSummary.pending += amount;
        if (s.status === 'overdue')
            statsSummary.overdue += amount;
    });
    return c.json({
